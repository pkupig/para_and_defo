...
DEFORM_METHODS = {
    0: "ARAP-style Smooth Deformation",
    1: "Stable Smooth Deformation",
}
...
class DeformationSession:
    ...
    def _ensure_anchor_vertices(self, exclude_vertex_id=None, target_count=3):
        """Create a few fixed anchors to remove rigid-body drift in deformation."""
        if self.anchor_vertices:
            return
        ...
        candidates = [i for i in range(n_vertices) if i != exclude_vertex_id]
        if not candidates:
            return

        seeds = []
        reference = exclude_vertex_id if exclude_vertex_id is not None else candidates[0]

        first = max(candidates, key=lambda idx: np.linalg.norm(self.vertices[idx] - self.vertices[reference]))
        seeds.append(first)

        if len(candidates) > 1:
            remaining = [idx for idx in candidates if idx not in seeds]
            if remaining:
                second = max(remaining, key=lambda idx: np.linalg.norm(self.vertices[idx] - self.vertices[first]))
                seeds.append(second)

        while len(seeds) < min(target_count, len(candidates)):
            remaining = [idx for idx in candidates if idx not in seeds]
            if not remaining:
                break
            next_idx = max(
                remaining,
                key=lambda idx: min(np.linalg.norm(self.vertices[idx] - self.vertices[s]) for s in seeds)
            )
            seeds.append(next_idx)

        for idx in seeds:
            self.anchor_vertices[idx] = np.array(self.vertices[idx], dtype=float)

    def _constraint_items(self):
        """Return stable deformation constraints with user handles overriding anchors."""
        constraints = {idx: pos.copy() for idx, pos in self.anchor_vertices.items()}
        for idx, pos in self.handles.items():
            constraints[idx] = np.array(pos, dtype=float)
        fix = list(constraints.keys())
        fix_vec = [constraints[idx] for idx in fix]
        return fix, fix_vec
    ...
    def set_handle(self, vertex_id, position):
        """
        """
        self._ensure_anchor_vertices(exclude_vertex_id=vertex_id)
    ...
    def reset(self):
        """Restore the original mesh and clear interaction state."""
        self.current_vertices = np.copy(self.vertices)
        self.handles.clear()
    ...
    def _arap_deform(self):
        # The original ARAP port is numerically unstable on many real meshes.
        # Use a stable control-point deformation for interactive visualization.
        return self._stable_handle_deform()
    
    def _laplacian_deform(self):
        return self._stable_handle_deform()

    def _stable_handle_deform(self, power=2.0):
        """
        Stable interactive deformation using inverse-distance blended handle offsets.
        This is used as a robust fallback suitable for demos and teaching.
        """
        fix, fix_vec = self._constraint_items()
        if not fix:
            return None

        original_vertices = np.asarray(self.vertices, dtype=float)
        control_indices = np.asarray(fix, dtype=int)
        control_targets = np.asarray(fix_vec, dtype=float)
        control_sources = original_vertices[control_indices]
        control_offsets = control_targets - control_sources

        new_vertices = np.array(original_vertices, copy=True)
        eps_local = 1e-12

        for vertex_id in range(len(original_vertices)):
            exact_match = np.where(control_indices == vertex_id)[0]
            if exact_match.size > 0:
                new_vertices[vertex_id] = control_targets[exact_match[0]]
                continue

            distances = np.linalg.norm(control_sources - original_vertices[vertex_id], axis=1)
            weights = 1.0 / np.power(np.maximum(distances, eps_local), power)
            weights_sum = np.sum(weights)
            if weights_sum <= eps_local:
                continue
            weights /= weights_sum
            blended_offset = weights @ control_offsets
            new_vertices[vertex_id] = original_vertices[vertex_id] + blended_offset

        if not np.isfinite(new_vertices).all() or np.max(np.abs(new_vertices)) > 1e6:
            logger.error("Stable deformation produced invalid vertex coordinates")
            return None
...
class DeformationServer:
    ...
    def reset(self, session_id):
        """Reset one deformation session to the loaded mesh state."""
        try:
            session = self.get_session(session_id)
            session.reset()
            return {
...
def parameterize():
    try:
        ...
        try:
            ...
            logger.error("Parameterization timed out")
...
def set_handle():
    ...
    if session_id is None or vertex_id is None or position is None:
...
def deform_mesh():
    ...
    if last_result is None:
        return jsonify({'error': 'No deformation result available'}), 400
    
    return jsonify({
        'vertices': last_result['vertices'],
        'metrics': last_result['metrics'],
        'iterations': iterations,
        'deform_method': last_result.get('deform_method', DEFORM_METHODS.get(method, "Unknown"))
    })


@app.route('/reset_deformation', methods=['POST'])
def reset_deformation():
    data = request.json
    session_id = data.get('session_id')
    ...
    result = deformation_server.reset(session_id)
    if result is None:
        return jsonify({'error': 'Failed to reset deformation'}), 500
...
if __name__ == '__main__':
    ...
    if not app.debug or is_reloader_worker:
