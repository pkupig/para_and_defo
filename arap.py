...
def getCovariance3x3(neighbor, verts, now_res, weights, cur):
    ...
    for i in range(len(neighbor)):
        ...
        w = weights[cur * verts.shape[0] + neighbor_idx]
        deformed_edge = now_res[cur] - now_res[neighbor_idx]
        original_edge = verts[cur] - verts[neighbor_idx]
        S += w * np.outer(deformed_edge, original_edge)
    return S
